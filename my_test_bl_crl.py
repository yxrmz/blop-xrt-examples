...
from xrt_beamline_crl import BeamlineEpics
...
beamline = BeamlineEpics('CRL:', name="bl") 
   
time.sleep(1)


dofs = [DOF(description=f"Lens{n:02d}.Z",
        device=getattr(beamline.lenses, f'Lens{n:02d}').z,
        search_domain=(-dR1, dR1)) for n in range(48)]

motor_names = (f'bl_lenses_Lens{n:02d}_z' for n in range(48))
# dofs = [
...
if isEPICS:
    digestion_kwargs.update({"image_shape": (256, 256)})
...
if len(best_image.shape) < 2:
    best_image = np.reshape(best_image, (256, 256))
